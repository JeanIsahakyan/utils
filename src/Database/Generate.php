...
class Generate {
  ...
  private array $object_names = [];

  private const TAB = '  ';



  public function __construct(AbstractDatabase $db, string $name, string $dir, string $namespace, $object_names = [], bool $forced = false) {
    $this->db   = $db;
    $this->dir  = $dir;
  ...
  public static function create(AbstractDatabase $db, string $name, string $dir, string $namespace, $object_names = [], bool $forced = false): self {
    return new self($db, $name, $dir, $namespace, $object_names, $forced);
  }
  ...
  private function getObjectName(string $table_name) {
    if (array_key_exists($table_name, $this->object_names)) {
      return $this->camelize($this->object_names[$table_name]);
    }
  ...
  private function generate(string $table_name, array $fields, array $indexes) {
    ...
    $object_class_name = $this->getObjectName($table_name);
